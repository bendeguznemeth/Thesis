...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    //UIElements
    //ExportButton is used to collect data for training
    ...
    //Training data was exported in CSV
    ...
    //Count and Sum for displaying result only on every 6th run
    var resultCount = 0
    var resultSum = 0.0
    ...
    override func viewDidLoad() {
        ...
        //Do not need the exportButton anymore
    ...
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        //Do not need to collect data anymore
        //if (!first) {
        //    let date = Date()
        ...
        let request = VNCoreMLRequest(model: model) { (finishedReq, err) in
            ...
            //We print the movement of the device instead
//            DispatchQueue.main.async {
//                self.label.text = "\(firstObservation.identifier) \(Int(firstObservation.confidence * 100))%"
//            }
        ...
        if (!first) {
            ...
            resultCount += 1
            resultSum += output.target
            if resultCount >= 6 {
                DispatchQueue.main.async {
                    self.label.text = "\(Int(self.resultSum)) mm"
                    self.resultCount = 0
                    self.resultSum = 0.0
                }
            }
